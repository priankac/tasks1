...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    const trimmed_answer = question.expected.trim().toLowerCase();
    return trimmed === trimmed_answer;
}
...
export function toShortForm(question: Question): string {
    const less = question.name.substring(0, 10);
    return question.id + ": " + less;
}
...
export function toMarkdown(question: Question): string {
    const first_option = "# " + question.name + "\n" + question.body;
    if (question.type === "multiple_choice_question") {
        const options_type = question.options
...
export function addOption(question: Question, newOption: string): Question {
    const add_option = [...question.options, newOption];
    return {
        ...question,
        options: add_option,
    };
